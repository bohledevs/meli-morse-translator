...
public interface MorseService {
  ...
  MorseDto translate2Human(
      @Pattern(regexp = "[\\.\\-\\s]+", message = "Only dots, dashes and whitespaces")
          String morse);

  MorseDto translate2Morse(
      @Pattern(regexp = "[A-Z0-9\\s]+", message = "Only alphanumerics and whitespaces")
          String human);
}
