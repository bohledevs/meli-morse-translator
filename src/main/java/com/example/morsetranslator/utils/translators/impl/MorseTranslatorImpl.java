...
import static com.example.morsetranslator.utils.Constants.DOUBLE_WHITESPACE;
        ...
        .map(
            word ->
                Arrays.stream(word.split("[ ]")) // split by letters (1 whitespace)
                    .map(
                        letter ->
                            this.morseConfig.getAlphabet().entrySet().stream()
            ...
            word ->
                Arrays.stream(word.split(EMPTY_STRING)) // split by letters (empty string)
                    .map(
                        letter ->
                            this.morseConfig.getAlphabet().entrySet().stream()
                                .filter(entry -> letter.equals(entry.getKey()))
                                .findFirst()
                                .orElse(
                                    new AbstractMap.SimpleEntry<String, String>(
                                        EMPTY_STRING, EMPTY_STRING))
                                .getValue())
                    .collect(Collectors.joining(WHITESPACE)))
        .collect(Collectors.joining(DOUBLE_WHITESPACE));
  }
}
